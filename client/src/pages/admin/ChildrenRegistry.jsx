  ...
  return (
    <Layout role="admin">
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <h4 className="fw-bold mb-0"><i className="bi bi-people-fill me-2 text-success"></i>Children Registry ({children.length})</h4>
      </div>
      <div className="row g-2 mb-3">
        <div className="col-md-6"><input className="form-control" placeholder="Search by name or district..." value={search} onChange={e=>setSearch(e.target.value)} /></div>
        ...
        <div className="card border-0 shadow-sm">
          <div className="table-responsive">
            <table className="table table-hover mb-0">
              <thead><tr><th>Name</th><th>Age</th><th>Gender</th><th>District</th><th>Block</th><th>Weight</th><th>Height</th><th>Status</th><th>Parent</th></tr></thead>
              <tbody>
                {filtered.length===0?<tr><td colSpan={9} className="text-muted text-center py-4">No children found.</td></tr>:filtered.map(c=>(
                  <tr key={c._id}><td className="fw-semibold">{c.name}</td><td>{c.ageInMonths}mo</td><td className="text-capitalize">{c.gender}</td><td>{c.district||'—'}</td><td>{c.block||'—'}</td><td>{c.currentWeight?`${c.currentWeight}kg`:'—'}</td><td>{c.currentHeight?`${c.currentHeight}cm`:'—'}</td><td><span className={`badge ${STATUS[c.nutritionStatus]}`}>{c.nutritionStatus}</span></td><td>{c.parentId?.name||'—'}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
