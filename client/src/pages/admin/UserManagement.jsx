  ...
  return (
    <Layout role="admin">
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <h4 className="fw-bold mb-0"><i className="bi bi-person-gear me-2 text-warning"></i>User Management ({users.length})</h4>
        ...
        <div className="card border-0 shadow-sm">
          <div className="table-responsive">
            <table className="table table-hover mb-0">
              ...
              <tbody>
                {users.length===0?<tr><td colSpan={7} className="text-muted text-center py-4">No users found.</td></tr>:users.map(u=>(
                  <tr key={u._id}>
                    ...
                    <td><button className={`btn btn-sm ${u.isActive?'btn-outline-danger':'btn-outline-success'}`} onClick={()=>toggle(u._id)} disabled={toggling===u._id}>{toggling===u._id?'...':u.isActive?'Deactivate':'Activate'}</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
